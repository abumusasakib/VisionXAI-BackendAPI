...
- **Status Code**: `200`
- **Body**: JSON object with the generated caption and optional token-level outputs and attention image. Example shape:
...
- `attention_image` is a base64-encoded PNG (if attention visualization was produced); it can be `null` if plotting is unavailable in the runtime.
